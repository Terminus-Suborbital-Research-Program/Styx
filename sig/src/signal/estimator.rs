...
impl MatchingEstimator {
    pub fn new(current_power_spectrum: Vec<f32>, expected_power_spectrum: Vec<f32>, max_shift: usize)-> Self {
        Self { 
    ...
    pub fn sliding_window_match(&mut self) -> (f32, usize) {
        ...
        let current_average = &mut self.current_power_spectrum;
        ...
        let window_den: f32 = sliding_window.iter().map(|bin| (bin - window_mean).powi(2)).sum::<f32>().sqrt();
        ...
        for i in 0..=*max_shift {
            ...
            let compare_den: f32 = compare_slice.iter().map(|bin| (bin - compare_mean).powi(2)).sum::<f32>().sqrt();
    ...
    pub fn chi_square(&mut self, best_score: f32, best_index: usize) -> f32 {
        let shift: usize = &self.max_shift / 2;
        let current_start = shift;
        let current_end = &self.current_power_spectrum.len() - shift;
        let match_len = &self.current_power_spectrum.len() - &self.max_shift;

        let mut window = &self.current_power_spectrum[current_start..current_end];
        ...
        let alpha = 0.05;
        let chi_sqr = chi_square::goodness_of_fit(window.clone(), compare_slice.clone(), alpha).unwrap();
    ...
    pub fn sliding_window_advanced(&mut self) -> (f32,isize) {
        let shift: isize = (&self.max_shift / 2) as isize;

        let full_len = self.current_power_spectrum.len();
        ...
        for i in -shift..=shift {
            ...
            let (window_start, window_end) = if i < 0 { (i.abs() as usize, full_len) } else { (0,full_len - i as usize)};
            let (compare_start, compare_end) = if i < 0 { (0 , (full_len as isize + i) as usize) } else { (i as usize ,full_len )};


            let window_slice = &self.current_power_spectrum[window_start..window_end];
            ...
            let window_den: f32 = window_slice.iter().map(|bin| (bin - window_mean).powi(2)).sum::<f32>().sqrt();
    ...
    pub fn chi_square_advanced(&mut self, best_score: f32, best_index: isize) -> f32 {
        let full_len = self.current_power_spectrum.len();

        let (window_start, window_end) = if best_index < 0 { (best_index.abs() as usize, full_len) } else { (0,full_len - best_index as usize)};
        let (compare_start, compare_end) = if best_index < 0 { (0 , (full_len as isize + best_index) as usize) } else { (best_index as usize ,full_len )};
        ...
        let alpha = 0.05;
        let chi_sqr = chi_square::goodness_of_fit(window_slice.clone(), compare_slice.clone(), alpha).unwrap();
    ...
    pub fn match_estimate(&mut self) {
        let (best_score, best_index) =  self.sliding_window_match();
        let chi_score = self.chi_square(best_score, best_index);
    }

    pub fn match_estimate_advanced(&mut self) -> f32 {
        let (best_score, best_index) =  self.sliding_window_advanced();
        let chi_square = self.chi_square_advanced(best_score, best_index);
